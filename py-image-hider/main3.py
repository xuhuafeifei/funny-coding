...
large_file_path = "input/kunkun.png"
small_file_path = "input/woshini-baba.png"
...
if step == 0:
    step = 1
for i in range(step, ss[0], step * 1):
    row = []
    for j in range(step, ss[1], step * 1):
...
Image.fromarray(small_scale_matrix).save("./shipin/scale.png")
...
for i in range(step, ls[0], step * 12):
    for j in range(step, ls[1], step * 12):
        x, y = pos_in_matrix()
        if cnt >= ss[0] * ss[1]:
            break
        large_matrix[i][j] = (
            small_scale_matrix)[x][y]
        cnt += 1

# 将large_matrix写入文件
# Image.fromarray(large_matrix).save("./image3/result-" + str(cnt) + "x" + str(ss[0]) + ".png")
Image.fromarray(large_matrix).save("./shipin/result-" + str(cnt) + "x" + str(ss[0]) + ".png")
